...
export default function MobileAppliedJobs() {
  ...
  const { data: appliedJobsData, isLoading: isLoadingAppliedJobs } = useApiGet<JobApplicationResponse>(
    `applied-candidates/candidate/${user?.id}${duration ? `?appliedDateFilter=${duration}` : ""}`,
    ["applied-jobs", duration],
  )

  // Shortlisted Jobs
  const { data: shortlistedJobsData, isLoading: isLoadingShortlistedJobs } = useApiGet<JobApplicationResponse>(
    `applied-candidates/candidate/${user?.id}?shortlist=true${duration ? `&appliedDateFilter=${duration}` : ""}`,
    ["shortlisted-jobs", duration],
  )

  // Saved Jobs
  const { data: savedJobsData, isLoading: isLoadingSavedJobs } = useApiGet<JobApplicationResponse>(
    `applied-candidates/candidate/${user?.id}?saved=true${duration ? `&appliedDateFilter=${duration}` : ""}`,
    ["saved-jobs", duration],
  )
  ...
  const isLoading = isLoadingAppliedJobs || isLoadingShortlistedJobs || isLoadingSavedJobs
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <Tabs defaultValue="applied" value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid grid-cols-3 w-full">
          ...
          <TabsTrigger value="saved" className="relative">
            Saved
            <Badge className="ml-1 h-5 w-5 p-0 flex items-center justify-center text-[10px]">{savedJobs.length}</Badge>
          </TabsTrigger>
        </TabsList>

        <TabsContent value="applied" className="mt-4 space-y-3 scroll-y max-h-[calc(100vh-200px)] overflow-y-auto">
          {appliedJobs.length > 0 ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 mt-1">You haven't applied to any jobs yet</p>
            </div>
          )}
        </TabsContent>

        <TabsContent value="shortlisted" className="mt-4 space-y-3 scroll-y max-h-[calc(100vh-200px)] overflow-y-auto">
          {shortlistedJobs.length > 0 ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 mt-1">You don't have any shortlisted jobs yet</p>
            </div>
          )}
        </TabsContent>

        <TabsContent value="saved" className="mt-4 space-y-3 scroll-y max-h-[calc(100vh-200px)] overflow-y-auto">
          {savedJobs.length > 0 ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 mt-1">You haven't saved any jobs yet</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
