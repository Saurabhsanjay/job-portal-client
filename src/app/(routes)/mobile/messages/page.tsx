...
import { Search, Send, ChevronLeft, Loader2, MessageSquare } from "lucide-react"
...
const transformMessages = (messages: MessageResponse[], currentUser: string) => {
  // Filter messages where currentUser is either sender or receiver
  const filteredMessages = messages?.filter(
    (msg) => msg.senderId._id === currentUser || msg.receiverId._id === currentUser,
  )
  ...
  filteredMessages.forEach((msg) => {
    ...
    const otherUserAvatar = otherUser.personalDetails.profilePicture || "/default-avatar.png"
    ...
    conversation.messages.push({
      ...
      sender: {
        id: msg.senderId._id,
        name: isSender ? "You" : `${msg.senderId.personalDetails.firstName} ${msg.senderId.personalDetails.lastName}`,
        avatar: msg.senderId.personalDetails.profilePicture || "/default-avatar.png",
        isYou: isSender,
      },
    })
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      ...
      try {
        ...
        const transformedData = transformMessages(conversationData?.data?.data?.data, currentUser)
      ...
      if (data.event === "message") {
        ...
        const newMessageObj: Message = {
          ...
          sender: {
            ...
            avatar: data?.senderId?.personalDetails?.profilePicture || "/default-avatar.png",
            isYou: isSender,
          },
        }
          ...
          {conversations.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Start a conversation with a recruiter or wait for them to contact you
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
            {activeConversation?.messages.map((message) => (
              <div key={message.id} className={cn("flex", message.sender.isYou ? "justify-end" : "justify-start")}>
                <div className={cn("flex max-w-[80%]", message.sender.isYou ? "flex-row-reverse" : "flex-row")}>
                  <Avatar className={cn("h-8 w-8", message.sender.isYou ? "ml-2" : "mr-2")}>
                  ...
                  <div>
                    ...
                    <div
                      className={cn("text-xs text-gray-500 mt-1", message.sender.isYou ? "text-right" : "text-left")}
                    >
                      {message.timestamp}
                    </div>
                  </div>
