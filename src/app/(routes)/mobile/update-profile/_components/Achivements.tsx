...
export default function AchievementsAndCertifications({
  achievements: items,
  setAchievements: setItems,
}: {
  achievements: AchievementOrCertification[];
  setAchievements: React.Dispatch<
    React.SetStateAction<AchievementOrCertification[]>
  >;
}) {
  const [editingItem, setEditingItem] =
    useState<AchievementOrCertification | null>(null);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingItem) {
      setItems(
        items.map((item) => (item._id === editingItem._id ? newItem : item))
      );
    } else {
        ...
        <Input
          ...
          defaultValue={
            editingItem?.issuedDate
              ? new Date(editingItem?.issuedDate).toISOString().split("T")[0]
              : ""
          }
          required
        />
